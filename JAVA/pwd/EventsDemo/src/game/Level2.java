...
public class Level2 extends GameLevel {
    ...
    public void populate(Game game){
        ...
        bounce.addCollisionListener(new ObjectInteraction(getSonic(),bounce));
        ...
        for (int i = 0; i<60;i++){
            ...
            pipeStart.addCollisionListener(new ObjectInteraction(getSonic(),bounce));
            pipeMain.addCollisionListener(new ObjectInteraction(getSonic(),bounce));
        }
        ...
        getEnemy().addCollisionListener(new Interactions(getSonic(),getEnemy(),getEnemy2(),getEnemy3(),getEggMan()));
        getEnemy3().setPosition(new Vec2(150,47));
        getEnemy3().addCollisionListener(new Interactions(getSonic(),getEnemy(),getEnemy2(),getEnemy3(),getEggMan()));

        for (int i=0;i<5;i++){
            enemy=new Enemy(this);
            enemy.setPosition(new Vec2(-120+i*70,15));
            enemy.addCollisionListener(new Interactions(getSonic(),enemy,getEnemy2(),enemy3,getEggMan()));
        }

        for (int i=0;i<5;i++){
            enemy3=new Enemy3(this);
            enemy3.setPosition(new Vec2(140+i*70,15+i*40));
            enemy3.addCollisionListener(new Interactions(getSonic(),enemy,getEnemy2(),enemy3,getEggMan()));
        }
//        getEggMan().setPosition(new Vec2(-80,90));
        ...
        for (int i = 0; i < 30; i++) {
            ...
            ring.addCollisionListener(new ObjectInteraction(getSonic(),bounce));
        }
        //health
        for (int i = 0; i<1;i++){
            health = new Health(this);
            health.setPosition(new Vec2(-200,5));
            health.addCollisionListener(new ObjectInteraction(getSonic(),bounce));
        }
